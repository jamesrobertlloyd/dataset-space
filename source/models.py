...
class Tree_c():
    ...
    def description(self):
        return 'RF %s %s' % (self.min_samples_leaf, self.criterion)
...
class GBM_c():
    ...
    def __init__(self, n_estimators=500, learn_rate=0.1, max_depth=3):
        ...
        self._model = GradientBoostingClassifier(loss='deviance', n_estimators=n_estimators, learn_rate=learn_rate, max_depth=max_depth)
...
class SGD_c():
    ...
    def predict_p(self, X_train, y_train, X_test): 
        return self._model.fit(X_train, y_train).predict_proba(X_test)
...
list_of_classifiers = [GaussianNaiveBayes_c()] + \
                      ...
                      [GBM_c(n_estimators, learn_rate, max_depth) for n_estimators in [100,300,500] for learn_rate in [0.01,0.1,1] for max_depth in [1,3,5]] + \
